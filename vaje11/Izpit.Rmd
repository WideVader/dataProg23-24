...
dslabs::murders %>% 
  mutate(Prva=str_sub(state,1,1),BLA=str_extract(state,".")) %>%  # BLA is a second option on doing this
  group_by(Prva) %>% 
  summarise(Pop=sum(population),Murders=sum(total)) %>% 
  mutate(Odstotek=Murders/Pop*100) %>% 
  arrange(desc(Odstotek))
...
text %>% str_extract_all("[A-Z][a-z]*") %>% unlist()

text %>% str_extract_all("[0-9]+") %>% unlist()
